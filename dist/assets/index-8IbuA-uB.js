...
`,a.style.fontWeight=t>20?t>40?700:600:500,a.style.color=s}};const A=(r,e)=>{const n=u.canvas.getBoundingClientRect();if(r==null){u.exports.handleMouse(-1,-1,5);return}const i=(r.clientX-n.left)/n.width,t=(r.clientY-n.top)/n.height;i>=0&&i<=1&&t>=0&&t<=1?u.exports.handleMouse(i,t,e):u.exports.handleMouse(-1,-1,e)};window.addEventListener("blur",()=>{b=1/0,A(null,0)});document.addEventListener("pointermove",r=>{A(r,0)});document.addEventListener("pointerdown",r=>{const e=r.target;if(!e||document.getElementById("debugContainer").contains(e))return;const n=r.button===2?3:1;A(r,n)});document.addEventListener("pointerup",r=>{const e=r.button===2?4:2;A(r,e)});u.canvas.style.touchAction="none";document.addEventListener("contextmenu",r=>r.preventDefault());if(u.isDebug){u.exports.debugBuildUiMetadata();const r=u.readStr(),e=JSON.parse(r),n=document.getElementById("debugContainer");e.buttons.forEach(i=>{const t=document.createElement("button");t.textContent=i.name,t.onclick=()=>u.exports.clickDebugUiButton(i.id),n.appendChild(t)}),e.sliders.forEach(i=>{const t=document.createElement("div");t.style.display="flex",t.style.flexDirection="column";const s=document.createElement("label");s.textContent=`${i.name}: ${i.val.toFixed(2)}`,s.style.fontSize="12px";const a=document.createElement("input");a.type="range",a.min=i.min,a.max=i.max,a.step=((i.max-i.min)/1e3).toString(),a.value=i.val,a.oninput=l=>{const c=parseFloat(l.target.value);s.textContent=`${i.name}: ${c.toFixed(2)}`,u.exports.changeDebugUiSlider(i.id,c)},t.appendChild(s),t.appendChild(a),n.appendChild(t)}),document.body.appendChild(n)}setTimeout(function(){u.renderLoop(0)},17);
