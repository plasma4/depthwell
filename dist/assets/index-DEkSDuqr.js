(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))i(t);new MutationObserver(t=>{for(const s of t)if(s.type==="childList")for(const o of s.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&i(o)}).observe(document,{childList:!0,subtree:!0});function r(t){const s={};return t.integrity&&(s.integrity=t.integrity),t.referrerPolicy&&(s.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?s.credentials="include":t.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function i(t){if(t.ep)return;t.ep=!0;const s=r(t);fetch(t.href,s)}})();const a={zoom:131072,drop:262144,minus:32768,plus:65536,up:2048,left:4096,down:8192,right:16384,k0:1,k1:2,k2:4,k3:8,k4:16,k5:32,k6:64,k7:128,k8:256,k9:512},E={player_pos:0,last_player_pos:16,player_chunk:32,player_velocity:48,camera_pos:64,last_camera_pos:80,camera_scale:96,camera_scale_change:104,depth:112,player_quadrant:120,frame:124,keys_pressed_mask:128,keys_held_mask:132,seed:144,seed2:208},M="abcdefghijklmnopqrstuvwxyz",b=26n;function z(n=100){if(n<=0)return"";const e=new Uint8Array(72);crypto.getRandomValues(e);let r=0n;const i=new DataView(e.buffer);for(let o=0;o<e.length;o+=8)r=r<<64n|i.getBigUint64(o);let t="",s=r%b**BigInt(n);for(;s>=0n&&(t+=M[Number(s%b)],s=s/b-1n,!(s<0n)););return t}function N(n){let e=0n;for(let r=0;r<n.length;r++){const i=BigInt(n.charCodeAt(r)-97);e=e*b+(i+1n)}return e}async function F(n,e){const r=N(n),i=new DataView(new ArrayBuffer(64));for(let c=0;c<8;c++)i.setBigUint64(c*8,r>>BigInt((7-c)*64)&0xffffffffffffffffn);let t=new Uint8Array(i.buffer,0,32),s=new Uint8Array(i.buffer,32,32);const o=await Promise.all([0,1,2,3].map(c=>crypto.subtle.importKey("raw",new Uint8Array([c]),{name:"HMAC",hash:"SHA-256"},!1,["sign"])));for(const c of o){const d=new Uint8Array(await crypto.subtle.sign("HMAC",c,s)),p=new Uint8Array(32);for(let f=0;f<32;f++)p[f]=t[f]^d[f];t=s,s=p}const _=new Uint8Array(64);return _.set(t,0),_.set(s,32),e.set(new BigUint64Array(_.buffer)),e}const R={Minus:a.minus,Equal:a.plus,KeyZ:a.zoom,KeyQ:a.drop,Space:a.up,ArrowUp:a.up,KeyW:a.up,ArrowLeft:a.left,KeyA:a.left,ArrowDown:a.down,KeyS:a.down,ArrowRight:a.right,KeyD:a.right,Digit0:a.k0,Digit1:a.k1,Digit2:a.k2,Digit3:a.k3,Digit4:a.k4,Digit5:a.k5,Digit6:a.k6,Digit7:a.k7,Digit8:a.k8,Digit9:a.k9};function V(){let n={};const e={heldMask:0,keysHeld:0,keysPressed:0,currentlyHeld:0,horizontalPriority:0,verticalPriority:0,plusMinusPriority:0};function r(){n={},e.horizontalPriority=0,e.verticalPriority=0,e.plusMinusPriority=0,e.currentlyHeld=0,e.heldMask=0,e.keysPressed=0}return window.addEventListener("keydown",i=>{if(i.repeat||i.altKey||i.ctrlKey||i.metaKey||i.shiftKey)return;const t=R[i.code];t&&(e.heldMask|=t,n[t]=(n[t]||0)+1,t&(a.left|a.right)&&(e.horizontalPriority=t),t&(a.up|a.down)&&(e.verticalPriority=t),t&(a.plus|a.minus)&&(e.plusMinusPriority=t))}),window.addEventListener("keyup",i=>{const t=R[i.code];t&&(n[t]=Math.max(0,(n[t]||0)-1),n[t]===0&&(e.heldMask&=~t,t===e.horizontalPriority&&(e.horizontalPriority=e.heldMask&a.left||e.heldMask&a.right||0),t===e.verticalPriority&&(e.verticalPriority=e.heldMask&a.up||e.heldMask&a.down||0),t===e.plusMinusPriority&&(e.plusMinusPriority=e.heldMask&a.plus||e.heldMask&a.minus||0)))}),window.addEventListener("blur",r),document.addEventListener("visibilitychange",r),window.addEventListener("contextmenu",r),e}function H(n){const e=a.up|a.down|a.left|a.right;let r=n.heldMask&~e;r|=n.horizontalPriority,r|=n.verticalPriority,r|=n.plusMinusPriority,n.keysPressed=r&~n.keysHeld,n.currentlyHeld=r,n.keysHeld=r}const q=""+new URL("main-941LRPO3.wasm",import.meta.url).href,C=`/*
...
fn vs_main(
    ...
    let sprite_col = f32(id % TILES_PER_ROW_U);
    let sprite_row = f32(id / TILES_PER_ROW_U);
...
fn fs_main(in: VertexOutput) -> @location(0) vec4f {
    ...
    let hp_col = f32(hp_id % TILES_PER_ROW_U);
    let hp_row = f32(hp_id / TILES_PER_ROW_U);
    let stone_uv = vec2f(hp_col * SPRITE_W, hp_row * SPRITE_H) + (safe_local_uv * vec2f(SPRITE_W, SPRITE_H));

    // Sample the primary color (the actual original sprite, gem or not)
    var hp_darkness_mult = textureSampleLevel(sprite_atlas, pixel_sampler, stone_uv, 0.0).r;
    var tex_color = textureSampleLevel(sprite_atlas, pixel_sampler, final_uv, 0.0) * vec4f(hp_darkness_mult, hp_darkness_mult, hp_darkness_mult, 1.0); // this works because the background is white beforehand from the sprites
    ...
    if (in.sprite_id >= GEM_START && in.sprite_id < GEM_MASK_START) {
        ...
        if ((extractBits(seed, 25u, 1u) == 1u)) {
            flipped_uv.x = 1.0 - flipped_uv.x;
        }

        // decide vertical for bit 27
        if ((extractBits(seed, 26u, 1u) == 1u)) {
        ...
        let bg_col = f32(bg_id % TILES_PER_ROW_U);
        let bg_row = f32(bg_id / TILES_PER_ROW_U);
        ...
        let mask_col = f32(mask_id % TILES_PER_ROW_U);
        let mask_row = f32(mask_id / TILES_PER_ROW_U);
    ...
    let extracted_l = f32(extractBits(seed, 0u, 3u));
    let extracted_a = f32(extractBits(seed, 3u, 3u));
    let l_nudge = extracted_l / 7.0;
    let a_nudge = extracted_a / 7.0;
    let b_nudge = f32(extractBits(seed, 6u, 3u)) / 7.0;
...
`,W=""+new URL("main-Sheet-DVtxt0kG.png",import.meta.url).href;async function K(n,e){const r=await navigator.gpu.requestAdapter({powerPreference:e&&e.highPerformance?"high-performance":"low-power"});if(!r)throw new DOMException("Couldn't request WebGPU adapter.","NotSupportedError");const i=await r.requestDevice();let t=null;if(i.addEventListener("uncapturederror",u=>{const g=u.error;if(t===null)if(globalThis.reportError)reportError(g);else throw g;else if(!t.destroyed){t.destroy("fatal WebGPU error",g);return}}),i.lost.then(u=>console.error(`WebGPU Device lost: ${u.message}`)),n===void 0){if(n=document.getElementsByTagName("canvas")[0],n===void 0)throw Error("No canvas element or ID string provided, and no canvas was not found in the HTML.")}else if(typeof n=="string"){const u=document.getElementById(n);if(!(u instanceof HTMLCanvasElement))throw Error(`Element with ID "${n}" is not a canvas element.`);n=u}const s=n.getContext("webgpu");if(!s)throw Error("Could not get WebGPU context from canvas.");const o=i.features.has("canvas-rgba16float-support")?"rgba16float":"bgra8unorm";s.configure({device:i,format:o,alphaMode:"opaque"});const _=await WebAssembly.instantiateStreaming(fetch(q),{env:{js_message:(u,g,v)=>{let h=new TextDecoder().decode(new Uint8Array(d.buffer,Number(u),Number(g)));h.charAt(0)!=="]"?h="["+(t?.LOGGING_PREFIX||"")+h:h=h.slice(1),v===1?console.info("%c"+h,"font-weight: 600"):[console.log,console.info,console.warn,console.error][v](h)},js_write_text:(u,g,v)=>{const h=new Uint8Array(d.buffer,Number(g),Number(v)),G=new TextDecoder().decode(h),U=document.getElementById(`text${u+1}`);U.textContent=G},js_get_time:()=>performance.now(),js_handle_visible_chunks:u=>t?.handleVisibleChunks(u)}}),c=_.instance.exports,d=c.memory,p=i.createShaderModule({label:"Main shader",code:C.replace("/* TILES_PER_ROW */ 1 /* TILES_PER_ROW */",""+c.get_tiles_per_row()).replace("/* TILES_PER_COLUMN */ 1 /* TILES_PER_COLUMN */",""+c.get_tiles_per_column()).replace("/* STONE_START */ 1 /* STONE_START */",""+c.get_stone_start()).replace("/* ORE_START */ 1 /* ORE_START */",""+c.get_ore_start()).replace("/* GEM_START */ 1 /* GEM_START */",""+c.get_gem_start()).replace("/* GEM_MASK_START */ 1 /* GEM_MASK_START */",""+c.get_gem_mask_start()).replace("/* DECOR_START */ 1 /* DECOR_START */",""+c.get_decor_start())}),f=i.createBindGroupLayout({label:"Main bind group layout",entries:[{binding:0,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:"uniform",hasDynamicOffset:!0}},{binding:1,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:"read-only-storage"}},{binding:2,visibility:GPUShaderStage.FRAGMENT,texture:{}},{binding:3,visibility:GPUShaderStage.FRAGMENT,sampler:{}}]}),m=i.createPipelineLayout({label:"Shared Pipeline Layout",bindGroupLayouts:[f]}),A=i.createRenderPipeline({label:"Tilemap pipeline",layout:m,vertex:{module:p,entryPoint:"vs_main"},fragment:{module:p,entryPoint:"fs_main",targets:[{format:o,blend:{color:{srcFactor:"src-alpha",dstFactor:"one-minus-src-alpha"},alpha:{srcFactor:"one",dstFactor:"one-minus-src-alpha"}}}]},primitive:{topology:"triangle-list",cullMode:"none"}}),I=i.createRenderPipeline({label:"Background pipeline",layout:m,vertex:{module:p,entryPoint:"vs_background"},fragment:{module:p,entryPoint:"fs_background",targets:[{format:o}]},primitive:{topology:"triangle-list"}});t=new w(n,r,i,s,_,A,I),t.exports.setup(),await t.setSeed(z(100)),t.startDelta=Number(c.mix_seed(60n)%120000n),t.exports.init();const B=new ResizeObserver(t.onResize);t.resizeObserver=B,t.updateCanvasStyle();try{t.resizeObserver.observe(n,{box:"device-pixel-content-box"})}catch{console.log("ResizeObserver property device-pixel-content-box not supported, falling back to content-box."),t.resizeObserver.observe(n,{box:"content-box"})}t.onResize([{contentRect:{width:n.clientWidth,height:n.clientHeight}}]);const O=await w.loadTexture(i,W),L=i.createSampler({magFilter:"nearest",minFilter:"nearest",addressModeU:"clamp-to-edge",addressModeV:"clamp-to-edge"});t.atlasTextureView=O.createView(),t.pixelSampler=L;const D=i.createBuffer({label:"SceneUniforms",size:256*T,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});return t.uniformBuffer=D,t}var k=(n=>(n[n.Uint8=8]="Uint8",n[n.Uint16=16]="Uint16",n[n.Uint32=32]="Uint32",n[n.Uint64=64]="Uint64",n[n.Int8=-8]="Int8",n[n.Int16=-16]="Int16",n[n.Int32=-32]="Int32",n[n.Int64=-64]="Int64",n[n.Uint8Clamped=-80]="Uint8Clamped",n[n.Float32=-320]="Float32",n[n.Float64=-640]="Float64",n))(k||{});globalThis.WasmTypeCode=k;const P={8:Uint8Array,16:Uint16Array,32:Uint32Array,64:BigUint64Array,[-8]:Int8Array,[-16]:Int16Array,[-32]:Int32Array,[-64]:BigInt64Array,[-80]:Uint8ClampedArray,[-320]:Float32Array,[-640]:Float64Array},T=4;class w{engineModule;exports;memory;LAYOUT_PTR;GAME_STATE_PTR;canvas;adapter;device;context;bindGroups=Array(T);uniformBuffer;tileBuffers=Array(T);tileBufferDirty=!1;atlasTextureView;pixelSampler;renderPipeline;bgPipeline;renderPass=null;currentEncoder=null;currentTextureView=null;renderCallId=0;sceneDataBuffer=new ArrayBuffer(256);sceneDataF32=new Float32Array(this.sceneDataBuffer);sceneDataU32=new Uint32Array(this.sceneDataBuffer);inputState;resizeObserver;forceAspectRatio=!0;previousForceAspectRatio=null;tileMapWidth;tileMapHeight;prepare_visible_chunks_time=0;isVisibleDataNew=!0;wireframeOpacity=0;startTime=performance.now();startDelta;seed="";destroyed=!1;destroyedError=null;encoder=new TextEncoder;decoder=new TextDecoder;LOGGING_PREFIX="";constructor(e,r,i,t,s,o,_){this.canvas=e,this.adapter=r,this.device=i,this.context=t,this.engineModule=s,this.renderPipeline=o,this.bgPipeline=_,this.exports=s.instance.exports,this.memory=s.instance.exports.memory,this.LAYOUT_PTR=Number(this.exports.get_memory_layout_ptr()),this.GAME_STATE_PTR=Number(this.getScratchView()[3]),this.inputState=V()}static async create(e,r){return await K(e,r)}destroy(e="unknown reason",r=null){this.resizeObserver.disconnect(),this.destroyed=e,this.destroyedError=r}static async loadTexture(e,r){const t=await(await fetch(r)).blob(),s=await createImageBitmap(t),o=e.createTexture({label:`Texture from  ${r}`,size:[s.width,s.height],format:e.features.has("canvas-rgba16float-support")?"rgba16float":"bgra8unorm",usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_DST|GPUTextureUsage.RENDER_ATTACHMENT});return e.queue.copyExternalImageToTexture({source:s},{texture:o},[s.width,s.height]),o}uploadVisibleChunks(e=1){const r=performance.now();this.exports.prepare_visible_chunks(e,this.canvas.width,this.canvas.height),this.prepare_visible_chunks_time=performance.now()-r}handleVisibleChunks(e){if(!this.currentEncoder||!this.currentTextureView||!this.renderPass)return;const r=this.getScratchPtr();if(this.getScratchLen()===0)return;const t=Number(this.getScratchProperty(0)),s=Number(this.getScratchProperty(1)),o=t*s*2;this.tileMapWidth=t,this.tileMapHeight=s;const _=new Uint32Array(this.memory.buffer,r,o),c=o*4;this.recreateBufferAndBindGroup(c),this.renderPass.setPipeline(this.renderPipeline),this.renderPass.setBindGroup(0,this.bindGroups[this.renderCallId],[this.renderCallId*256]),this.renderPass.setViewport(0,0,this.canvas.width,this.canvas.height,0,1),this.setSceneData(e,t,s),this.device.queue.writeBuffer(this.tileBuffers[this.renderCallId],0,_);const d=t*s+1;this.renderPass.draw(6,d),this.renderCallId++}setSceneData(e,r,i){const t=this.getScratchProperty(2,-640),s=this.getScratchProperty(3,-640),o=this.getScratchProperty(4,-640),_=this.getScratchProperty(5,-640),c=this.getScratchProperty(6,-640);this.sceneDataF32[0]=t,this.sceneDataF32[1]=s,this.sceneDataF32[2]=this.canvas.width,this.sceneDataF32[3]=this.canvas.height;const d=6e4,f=(performance.now()-this.startTime+this.startDelta)%(d*2);let m;f<d?m=f/1e3:m=(d-(f-d))/1e3,this.sceneDataF32[4]=m,this.sceneDataF32[5]=o,this.sceneDataF32[6]=o<.25?0:this.wireframeOpacity,this.sceneDataF32[7]=e,this.sceneDataF32[8]=_,this.sceneDataF32[9]=c,this.sceneDataU32[10]=r,this.sceneDataU32[11]=i,this.device.queue.writeBuffer(this.uniformBuffer,this.renderCallId*256,this.sceneDataF32)}recreateBufferAndBindGroup(e){const r=this.renderCallId;(!this.tileBuffers[r]||this.tileBuffers[r].size<e)&&(this.tileBuffers[r]=this.device.createBuffer({label:`Tile grid slot ${r}`,size:e,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.bindGroups[r]=this.device.createBindGroup({label:`Bind group slot ${r}`,layout:this.renderPipeline.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.uniformBuffer,offset:0,size:256}},{binding:1,resource:{buffer:this.tileBuffers[r]}},{binding:2,resource:this.atlasTextureView},{binding:3,resource:this.pixelSampler}]}))}getWASMMemoryMB(){return this.memory.buffer.byteLength/1024/1024}getGameView(e,r=0,i){return new P[e](this.memory.buffer,this.GAME_STATE_PTR+r,i)}getRawView(e,r,i){return new P[e](this.memory.buffer,r,i)}_tempScratchViewU64=null;_tempScratchViewF64=null;getScratchView(){return(this._tempScratchViewU64===null||this._tempScratchViewU64.buffer!==this.memory.buffer)&&(this._tempScratchViewU64=new BigUint64Array(this.memory.buffer,this.LAYOUT_PTR,24)),this._tempScratchViewU64}getScratchPtr(){return Number(this.getScratchView()[0])}getScratchLen(){return Number(this.getScratchView()[1])}setScratchLen(e){this.getScratchView()[1]=BigInt(e)}getScratchCapacity(){return Number(this.getScratchView()[2])}getScratchProperty(e,r=64){(this._tempScratchViewU64===null||this._tempScratchViewU64.buffer!==this.memory.buffer)&&(this._tempScratchViewU64=new BigUint64Array(this.memory.buffer,this.LAYOUT_PTR,24));let i=this._tempScratchViewU64;return r==-640&&((this._tempScratchViewF64===null||this._tempScratchViewF64.buffer!==this.memory.buffer)&&(this._tempScratchViewF64=new Float64Array(i.buffer,i.byteOffset,i.length)),i=this._tempScratchViewF64),Number(i[e+4])}readStr(e=this.getScratchPtr(),r=this.getScratchLen()){const i=new Uint8Array(this.memory.buffer,e,r);return this.decoder.decode(i)}writeStr(e,r=!0){const i=e.length;if(i===0)return null;r&&this.setScratchLen(0);const t=this.exports.scratch_alloc(i);if(t===0n)return null;const s=new Uint8Array(this.memory.buffer,Number(t),i);if(this.encoder.encodeInto(e,s).read<i)throw new RangeError("String truncated with non-ASCII characters detected.");return Number(t)}async setSeed(e){this.seed=e,await F(e,this.getGameView(64,E.seed,8))}updateCanvasStyle(){this.forceAspectRatio!==this.previousForceAspectRatio&&(this.previousForceAspectRatio=this.forceAspectRatio,this.forceAspectRatio?(this.canvas.style.maxWidth=`calc(100vh*${16/9})`,this.canvas.style.maxHeight=`calc(100vw*${9/16})`):(this.canvas.style.maxWidth="none",this.canvas.style.maxHeight="none"))}onResize=e=>{const r=e[0];let i,t;if(r.devicePixelContentBoxSize)i=r.devicePixelContentBoxSize[0].inlineSize,t=r.devicePixelContentBoxSize[0].blockSize;else if(r.contentBoxSize){const s=r.contentBoxSize[0].inlineSize,o=r.contentBoxSize[0].blockSize;i=Math.round(s*devicePixelRatio),t=Math.round(o*devicePixelRatio)}else{const s=r.contentRect.width,o=r.contentRect.height;i=Math.round(s*devicePixelRatio),t=Math.round(o*devicePixelRatio)}(this.canvas.width!==i||this.canvas.height!==t)&&(this.canvas.width=i,this.canvas.height=t)};renderFrame(e,r){if(this.renderCallId=0,this.destroyed!==!1)return;this.updateCanvasStyle(),this.currentEncoder=this.device.createCommandEncoder(),this.currentTextureView=this.context.getCurrentTexture().createView();const i=this.currentEncoder.beginRenderPass({colorAttachments:[{view:this.currentTextureView,loadOp:"clear",clearValue:{r:0,g:0,b:0,a:1},storeOp:"store"}]});this.renderPass=i,this.recreateBufferAndBindGroup(1024),this.sceneDataF32[7]=1,this.device.queue.writeBuffer(this.uniformBuffer,this.renderCallId*256,this.sceneDataF32),this.renderPass.setPipeline(this.bgPipeline),this.renderPass.setBindGroup(0,this.bindGroups[this.renderCallId],[this.renderCallId++*256]),this.renderPass.draw(3),this.uploadVisibleChunks(e),this.renderPass.end(),this.device.queue.submit([this.currentEncoder.finish()]),this.currentEncoder=null,this.currentTextureView=null}tick(e,r){const i=this.getGameView(32,E.keys_pressed_mask,2);H(this.inputState),i[0]=this.inputState.keysPressed,i[1]=this.inputState.keysHeld,this.exports.tick(e,r)}}location.protocol==="file:"&&alert("This game cannot run from a local file:// context; use an online version or test from localhost instead.");isSecureContext||alert("This game cannot run in a non-secure context.");navigator.gpu||alert("WebGPU is not supported by your browser; try playing this on an alternate or more modern browser.");const Z=await navigator.gpu.requestAdapter();Z||alert("WebGPU is supported, but no compatible GPU was found.");globalThis.Zig={KeyBits:a,game_state_offsets:E};console.log('Note: engine is in verbose mode, but Zig code is not in -Doptimize=Debug; run just "zig build" to enable additional testing features and safety checks if possible.');document.addEventListener("wheel",function(n){n.ctrlKey},{passive:!1});let l=await w.create();l.getTimeoutLength=function(){return++X%3==2?16:17};l.getFrameRate=function(){return 60};l.baseSpeed=1;let x=performance.now(),y=0,X=0;globalThis.engine=l;console.log("Engine initialized successfully:",l),console.log("Exported functions and memory:",l.exports);window.addEventListener("blur",()=>x=1/0);Array(60).fill(0);Array(60).fill(0);Array(60).fill(0);l.isDebug=!!l.exports.isDebug();l.renderLoop=function(n){let e=performance.now(),r=x===1/0?0:e-x,i=Math.min(r*l.getFrameRate()/1e3,l.getFrameRate());l.logicLoop(Math.max(Math.floor(y+i),1)),y=(y+i)%1;let t=Math.min(y-1,0);l.renderFrame(t,x),requestAnimationFrame(l.renderLoop)};l.logicLoop=function(n){performance.now(),l.tick(60/l.getFrameRate()*l.baseSpeed,n),x=performance.now()};const S=(n,e)=>{if(n.target!==l.canvas)return;const r=l.canvas.getBoundingClientRect(),i=(n.clientX-r.left)/r.width,t=(n.clientY-r.top)/r.height;i>=0&&i<=1&&t>=0&&t<=1&&l.exports.handle_mouse(i,t,e)};document.addEventListener("pointermove",n=>{n.buttons>0&&S(n,0)});document.addEventListener("pointerdown",n=>{const e=n.button===2?3:1;S(n,e)});document.addEventListener("pointerup",n=>{const e=n.button===2?4:2;S(n,e)});l.canvas.style.touchAction="none";document.addEventListener("contextmenu",n=>n.preventDefault());setTimeout(function(){l.renderLoop(0)},17);
