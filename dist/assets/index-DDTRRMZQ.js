...
(Object state hidden: circular reference)`}alert(l)};window.onerror=(e,n,i,t,s)=>{r(s||e,n,i,t)},window.onunhandledrejection=e=>{r(e.reason)},console.error=(...e)=>{const n=e.find(i=>i instanceof Error)||e[0];r(n)}}document.addEventListener("wheel",function(r){r.ctrlKey},{passive:!1});let u=await T.create();u.getTimeoutLength=function(){return++te%3==2?16:17};u.getFrameRate=function(){return 60};u.baseSpeed=1;let x=performance.now(),y=0,te=0;globalThis.engine=u;const R=Array(60).fill(0),P=Array(60).fill(0),A=Array(60).fill(0);u.isDebug=!!u.exports.isDebug();u.renderLoop=function(r){let e=performance.now(),n=x===1/0?0:e-x;x=e;const i=1e3/u.getFrameRate(),t=n/i,s=Math.min(y+t,5);let a=Math.floor(s);a>0?(u.logicLoop(a),y=s-a):y=s;{P.shift(),P.push(n),A.shift(),A.push(u.prepare_visible_data_time);const c=Math.max.apply(null,P),d=Math.max.apply(null,A);let h="#cccccc";c>55?h="#e83769":c>30?h="#f39c19":c>20&&(h="#f7ce1a");const f=document.getElementById("renderText");f.textContent=`Time since last render/prepare_visible_data time: ${n.toFixed(1)}ms, ${u.prepare_visible_data_time.toFixed(1)}ms
...
`,a.style.fontWeight=t>20?t>40?700:600:500,a.style.color=s}};const E=(r,e)=>{const n=u.canvas.getBoundingClientRect();if(r==null){u.exports.handleMouse(-1,-1,5);return}const i=(r.clientX-n.left)/n.width,t=(r.clientY-n.top)/n.height;i>=0&&i<=1&&t>=0&&t<=1?u.exports.handleMouse(i,t,e):u.exports.handleMouse(-1,-1,e)};window.addEventListener("blur",()=>{x=1/0,E(null,0)});document.addEventListener("pointermove",r=>{E(r,0)});document.addEventListener("pointerdown",r=>{const e=r.target;if(!e||document.getElementById("debugContainer").contains(e))return;const n=r.button===2?3:1;E(r,n)});document.addEventListener("pointerup",r=>{const e=r.button===2?4:2;E(r,e)});u.canvas.style.touchAction="none";document.addEventListener("contextmenu",r=>r.preventDefault());if(u.isDebug){u.exports.debugBuildUiMetadata();const r=u.readStr(),e=JSON.parse(r),n=document.getElementById("debugContainer");e.buttons.forEach(i=>{const t=document.createElement("button");t.textContent=i.name,t.onclick=()=>u.exports.clickDebugUiButton(i.id),n.appendChild(t)}),e.sliders.forEach(i=>{const t=document.createElement("div");t.style.display="flex",t.style.flexDirection="column";const s=document.createElement("label");s.textContent=`${i.name}: ${i.val.toFixed(2)}`,s.style.fontSize="12px";const a=document.createElement("input");a.type="range",a.min=i.min,a.max=i.max,a.step=((i.max-i.min)/1e3).toString(),a.value=i.val,a.oninput=l=>{const c=parseFloat(l.target.value);s.textContent=`${i.name}: ${c.toFixed(2)}`,u.exports.changeDebugUiSlider(i.id,c)},t.appendChild(s),t.appendChild(a),n.appendChild(t)}),document.body.appendChild(n)}setTimeout(function(){u.renderLoop(0)},17);
