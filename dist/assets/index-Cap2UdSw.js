...
fn oklch_to_oklab(lch: vec3f) -> vec3f {
    ...
    <div id="debugContainer"></div>`;document.addEventListener("wheel",function(i){i.ctrlKey},{passive:!1});let c=await P.create();c.getTimeoutLength=function(){return++j%3==2?16:17};c.getFrameRate=function(){return 60};c.baseSpeed=1;let v=performance.now(),y=0,j=0;globalThis.engine=c;console.log("Engine initialized successfully:",c),console.log("Exported functions and memory:",c.exports);window.addEventListener("blur",()=>v=1/0);const T=Array(60).fill(0),w=Array(60).fill(0),S=Array(60).fill(0);c.isDebug=!!c.exports.isDebug();c.renderLoop=function(i){let t=performance.now(),n=v===1/0?0:t-v,r=Math.min(n*c.getFrameRate()/1e3,c.getFrameRate());c.logicLoop(Math.max(Math.floor(y+r),1)),y=(y+r)%1;{w.shift(),w.push(n),S.shift(),S.push(c.prepare_visible_data_time);const s=Math.max.apply(null,w),o=Math.max.apply(null,S);let u="#cccccc";s>55?u="#e83769":s>30?u="#f39c19":s>20&&(u="#f7ce1a");const l=document.getElementById("renderText");l.textContent=`Time since last render/prepare_visible_data time: ${n.toFixed(1)}ms, ${c.prepare_visible_data_time.toFixed(1)}ms
