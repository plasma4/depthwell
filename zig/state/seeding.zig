...
pub fn mixCoordinateSeed(layer_seed: *const Seed, x: u64, y: u64, depth: u64) Seed {
    const PackedInput = extern struct { // temporary struct for faster mixing :)
        seed: Seed,
        x: u64,
        y: u64,
...
/// If `oddsNum(0.2)` is called, then there is a 20% chance that that integer is no larger than the value generated by this function.
/// For the case of 0.2, the number returned is 3689348814741910323. Floats are evaluated at 128-bit precision at comptime.
