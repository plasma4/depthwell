...
pub fn addRawEntity(entity: WGSLEntity) void {
    ...
    // Culling in UV, because WGSLEntity is already UV: the screen is 0 to 1 on both axes.
    // @abs because a horizontal flip negates size.x.
    const size = @abs(entity.size);

    // A rotation by angle a puts the half-extent at (w*|cos a| + h*|sin a|) / 2 on each axis,
    // which peaks at (w + h) / 2. Use the peak: the two axes have different UV scales, so a
    // half-diagonal of the WIDTH alone understates the reach of a tall sprite and clips it early.
    const half_extent = if (entity.rotation == 0.0)
        size * @as(Vec2f32, @splat(0.5))
    else
        @as(Vec2f32, @splat((size[0] + size[1]) * 0.5));

    const min_x = entity.position[0] - half_extent[0];
    const max_x = entity.position[0] + half_extent[0];
    const min_y = entity.position[1] - half_extent[1];
    const max_y = entity.position[1] + half_extent[1];
...
/// Draws `entity` over a larger copy of itself, which shows as an outline of `width_px` viewport pixels on every side.
///
...
/// Each tile layer uses the budget.
/// Past it the extras are dropped rather than growing the buffer mid-frame.
const MAX_BLOCK_ENTITIES = 256;

/// Block-anchored entities the current tile pass queued, already in viewport pixels.
var block_entities: [MAX_BLOCK_ENTITIES]Entity = undefined;
var block_entity_count: usize = 0;

/// Empties the queue. `chunks.updateVisibleChunks()` calls it once before the first tile layer runs.
...
/// `renderBlockEntities()` drains the queue after its tile layer, so each layer's overlay stays
/// over that layer and under the next layer.
///
...
pub fn queueBlockEntity(entity: BlockEntity, center_px: Vec2f32, zoom: f32, opacity: f32) void {
    ...
    const unwarped_size = entity.size * zoom;
    const unwarped_position = center_px + entity.offset * @as(Vec2f32, @splat(zoom));
    const position = dw.chunks.warpPosition(unwarped_position);
    const size = unwarped_size * dw.chunks.warpScale();
    ...
    block_entities[block_entity_count] = .{
        ...
        .position = position,
        .size = size,
        .rotation = entity.rotation + dw.chunks.warpRotation(),
        .lcha = entity.lcha * Vec4f32{ 1.0, 1.0, 1.0, opacity },
    };
    block_entity_count += 1;
}

/// Draws and clears the block overlays queued by one tile layer.
/// The caller must do this before another layer writes the scratch buffer.
pub fn renderBlockEntities() void {
    memory.scratchReset();
    entity_count = 0;
...
/// Precondition: `frame` is non-negative.
