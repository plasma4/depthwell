//! Regenerates `zig/render/particle_colors.zig` from the exported sprite atlas at build time.
//!
    ...
    for (0..tile_count) |tile| {
        ...
        for (0..TILE_SIZE) |py| {
            for (0..TILE_SIZE) |px| {
                ...
                if (!texel.isOpaque()) continue;
                appendUnique(&unique, &unique_len, texel);
            }
        }
...
/// Appends `color` to the tile's dedup scratch list unless an identical color is already present.
fn appendUnique(unique: []ColorRgba, unique_len: *usize, color: ColorRgba) void {
    for (unique[0..unique_len.*]) |seen| {
        if (seen.eql(color)) return;
    }
