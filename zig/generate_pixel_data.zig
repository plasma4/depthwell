...
pub fn main(init: std.process.Init) !void {
    ...
    // Most common (mode) opaque color per tile; white for fully-blank tiles.
    var dominant = try allocator.alloc(ColorRgba, tile_count);
    ...
    for (0..tile_count) |tile| {
        ...
        // Pick the mode (most common color) BEFORE sorting, while `counts` still aligns with `unique`.
        // Fully-blank tiles have no color at all, so fall back to opaque white.
        dominant[tile] = if (unique_len == 0) ColorRgba.white else blk: {
            var best: usize = 0;
            for (1..unique_len) |i| {
                if (counts[i] > counts[best]) best = i;
            }
            break :blk unique[best];
        };
    ...
    try writer.writeAll(
        ...
        \\pub const dominant = [TILE_COUNT][4]f32{
        \\
    );
